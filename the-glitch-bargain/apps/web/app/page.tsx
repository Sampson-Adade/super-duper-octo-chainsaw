          ...
          {room.phase === 'LOBBY' ? <>
            ...
            {room.phase === 'HACKING' && me?.alive && !me.queuedForNextRound && !submitted && <>
              <div className="terminal">
                <div className="terminalbar"><span className="dots"><i/><i/><i/></span><span>exploit.glitch</span><span className="level">{erasureActive ? 'ERASURE ACTIVE' : raid ? `${room.requiredCodeLines} LINES · SYNTAX LVL ${room.syntaxLevel}` : solo ? `${room.requiredCodeLines} LINES REQUIRED` : `SYNTAX LVL ${room.syntaxLevel}`}</span></div>
              ...
              {raid && <div className="requiredCode nonCopyable" onCopy={(event) => event.preventDefault()} onCut={(event) => event.preventDefault()} onContextMenu={(event) => event.preventDefault()} onDragStart={(event) => event.preventDefault()}><div className="eyebrow">YOUR ASSIGNED LINES · FOLLOW THE NUMBER</div><ol>{raidCodeLines.map((line, index) => <li key={`${index}-${line}`}><code>{line}</code><small>{index < 3 && raidAssignment?.playerId === room.raidAssignments[0]?.playerId ? `LEAD LINE ${index + 1}` : `YOUR LINE ${index + 1}`}</small></li>)}</ol><small>{room.syntaxLevel === 1 ? 'Level 1: the lead writes three opening lines; everyone writes two personal lines.' : room.syntaxLevel === 2 ? 'Level 2: each operative adds a physical claim and a target condition. Claims may be challenged.' : 'Level 3: each operative redirects the target and overrides the purge.'}</small></div>}
            ...
            {room.phase === 'REVIEW' && <div className="review"><div className="eyebrow">SUBMISSIONS // OPEN FOR REVIEW</div>{room.missedSubmissionPlayerIds.length > 0 && <div className="missedSubmissionNotice"><b>TIME RAN OUT</b><span>{room.players.filter((player) => room.missedSubmissionPlayerIds.includes(player.id)).map((player) => player.name).join(', ')} missed the submission and will lose 30 points.</span></div>}{room.submissions.map((submission) => <article key={submission.playerId}><b>{room.players.find((player) => player.id === submission.playerId)?.name}</b><pre>{submission.source}</pre>{!submission.valid && <small>SYNTAX ERRORS: {submission.errors.join(' ')}</small>}</article>)}</div>}
